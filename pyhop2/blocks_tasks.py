"""
Top-level file for blocks_tasks.
...
def main():
    ...
    plan = pyhop2.find_plan(state1,[('get','b')], verbose=1)
    check_result(plan,False)
...
""")
    ...
    plan = pyhop2.find_plan(state1,[('get','a')], verbose=1)
    check_result(plan, [('unstack','a', 'b')])

    plan = pyhop2.find_plan(state1,[('get','c')], verbose=1)
    check_result(plan, [('pickup','c')])

    plan = pyhop2.find_plan(state1,[('move_one','a','table')], verbose=1)
...
state variables. Below, both goal1a and goal1b say that we want the blocks in
the configuration "c on b on a", and goal1a also says we want "a on table".
...
""")
    ...
    plan1 = pyhop2.find_plan(state1,[('move_blocks', goal1a)], verbose=1)
    check_result(plan1,expected)

    plan2 = pyhop2.find_plan(state1,[('move_blocks', goal1b)], verbose=1)
...
#     print("('move_blocks', goal2b), it produces the same plan for both:")
    ...
    plan1 = pyhop2.find_plan(state2,[('move_blocks', goal2a)], verbose=1)
    check_result(plan1,expected)

    plan2 = pyhop2.find_plan(state2,[('move_blocks', goal2b)], verbose=1)
    check_result(plan2,expected)
    pause()
    ...
    plan = pyhop2.find_plan(state3,[('move_blocks', goal3)], verbose=1)
...
""")
    ...
    new_state = pyhop2.run_lazy_lookahead(state2, [('move_blocks', goal2b)], verbose=1)

    pause()

    print("The goal should now be satisfied, so the planner should return an empty plan:\n")

    plan = pyhop2.find_plan(new_state, [('move_blocks', goal2b)], verbose=1)
    check_result(plan,[])
